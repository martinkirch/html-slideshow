/*
 * HTML Slideshow
 * Authors : Rob Flaherty, Martin Kirchgessner
 * Copyright (c) 2011 Rob Flaherty 
 * Copyright (c) 2012,2013 Martin Kirchgessner
...
var htmlSlides = {
  ...
  init: function(options) {
    ...
    //Bind control events
    var actionsProxy = $.proxy(this, 'showActions')
    this.prevButton.bind('click', $.proxy(this, 'prevSlide'));
    this.nextButton.bind('click', actionsProxy);
    $('html').bind('keydown', $.proxy(this, 'keyControls'));
    $('html').bind('click', function(event) {
      if ($(event.target).is('section,#deck,p,li,h1,h2'))
        actionsProxy();
    });
